...
def blank_audio(audio_file, output_path):
    # load audio
    audio, sr = sf.read(audio_file)  # audio: np.ndarray, shape [n_samples] or [n_samples, n_channels]

    blank = np.zeros_like(audio)

    sf.write(output_path, blank, sr)
...
class DistortAudioFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.corruption == 'gaussian_noise':
            ...
            blank_audio(self.audio_paths[index], os.path.join(save_path, self.candi_audio_names[index]))
        else:
...
parser.add_argument('--corruption', type=str, default='gaussian_noise', choices=['all', 'gaussian_noise', 'traffic', 'crowd', 'rain', 'thunder', 'wind', 'missing'], help='Type of corruption to apply')
...
if args.corruption == 'all':
    corruption_list = ['traffic', 'crowd', 'rain', 'thunder', 'wind', 'gaussian_noise', 'missing']
else:
