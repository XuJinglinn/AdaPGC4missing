...
def missing(x, severity=1):
    width, height = x.size
    zero_image = Image.fromarray(np.zeros((height, width, 3), dtype=np.uint8), mode='RGB')
...
def save_distorted_for_image(method, candi_image_names, severity, data_path, save_path):
    ...
    for frame in range(10):
        ...
        for _ in tqdm(distorted_dataset_loader, desc='Processing', total=len(distorted_dataset_loader)):
            continue
...
parser.add_argument('--corruption', type=str, default='gaussian_noise', choices=['all', 'gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur', 'glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog', 'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression', 'missing'], help='Type of corruption to apply')
