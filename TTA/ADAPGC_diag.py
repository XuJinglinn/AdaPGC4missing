...
class IncrementalGMM:
    """
    Incremental diagonal Gaussian Mixture Model with soft-label batch updates
    and robust diagonal-Cholesky-based prediction.
    Maintains online statistics for each component and ensures positive variances.
    """
    def __init__(self, n_components, n_features, init_mu, init_bias, alpha=0.9, reg_covar=1e-6, dtype=torch.float32, device=None):
        ...
        if init_mu is not None:
            init_mu = init_mu.to(dtype=self.dtype, device=self.device)  # [K, d]
        ...
        # Diagonal covariance representation: [K, d].  Each row stores only
        # diag(Sigma_k), avoiding the O(K * d^2) dense allocation.
        self.covariances_ = torch.ones(self.K, self.d, dtype=dtype, device=self.device)
        # Compute robust diagonal Cholesky factors.
    ...
    def _update_cholesky(self):
        """
        Compute diagonal Cholesky factors with jitter to ensure positive-definiteness.

        covariances_: [K, d], storing diag(Sigma_k)
        cholesky_L:   [K, d], storing sqrt(diag(Sigma_k))
        """
        cov = self.covariances_.clone()
        jitter = self.reg_covar
        for attempt in range(5):
            if torch.isfinite(cov).all() and torch.all(cov > 0):
                self.cholesky_L = torch.sqrt(cov)
                self.log_det_cov_ = torch.sum(torch.log(cov), dim=-1)
                return
            cov = cov + jitter
            jitter *= 10
    ...
    def update_batch(self, X, gamma):
        ...
        delta_Sk = gamma.t() @ X
        # Only the diagonal of the weighted second moment is required:
        # delta_Qk[k, i] = sum_b gamma[b, k] * X[b, i]^2.
        delta_Qk = torch.einsum('bk,bi,bi->ki', gamma, X, X)
        self.Nk += delta_Nk
        self.Sk += delta_Sk
        # self.Qk += delta_Qk
        self.N += delta_Nk.sum()
        ...
        for k in range(self.K):
            self.covariances_[k] = alpha * self.covariances_[k] + (1 - alpha) * (
                ((self.covariances_[k] + means[k].square()) * self.Nk[k] + delta_Qk[k])
                / self.Nk[k] - self.means_[k].square()  # [d]
            )
        self._update_cholesky()
    ...
    def predict_batch(self, X, device=None):
        ...
        # For diagonal Sigma_k, the Mahalanobis distance is the elementwise
        # sum of squared residuals divided by the diagonal variances.
        # cholesky_L.square() is the jitter-stabilized diagonal covariance.
        variances = self.cholesky_L.square().unsqueeze(0)  # [1, K, d]
        maha = (diff.square() / variances).sum(dim=-1)     # [B, K]
    ...
    def get_params(self):
        """Return weights, means, and diagonal covariance vectors."""
...
# def init_cross_stats(K, dim, device, dtype):
    ...
    assert gmm_v.d == dim and gmm_f.d == dim, (
        "Diagonal cross-covariance requires audio, video, and fused features "
        "to have the same dimension"
    )

    # Diagonal cross-covariance representation: [K, dim].  Ones correspond
    # to the diagonal of the identity matrices used by the dense version.
    identity_diag = torch.ones(K, dim, dtype=dtype, device=device)
    ...
    cross_stats = {
        ...
        "Sigma_AF": identity_diag.clone(),
        "Sigma_VF": identity_diag.clone(),
    }
...
def update_cross_stats_from_full(cross_stats, gmm_a, gmm_v, gmm_f, ca, cv, feat_full, gamma_full, alpha):
    ...
    assert ca.shape == feat_full.shape and cv.shape == feat_full.shape, (
        "Diagonal cross-covariance requires ca, cv, and feat_full to have "
        "matching [batch, feature] shapes"
    )

    # Keep only matching-dimension cross moments:
    # delta_Q_XF[k, i] = sum_b gamma[b, k] * X[b, i] * F[b, i].
    delta_Q_AF = torch.einsum("bk,bi,bi->ki", gamma_full, ca, feat_full)  # [K, dim]
    delta_Q_VF = torch.einsum("bk,bi,bi->ki", gamma_full, cv, feat_full)  # [K, dim]
    ...
    for k in range(K):
        cross_stats["Sigma_AF"][k] = alpha * cross_stats["Sigma_AF"][k] + (1 - alpha) *(
            ((cross_stats["Sigma_AF"][k] + old_mean_A[k] * old_mean_F[k]) * new_Nk[k] + delta_Q_AF[k])
            / new_Nk[k] 
            - gmm_a.means_[k] * gmm_f.means_[k]
        )
        cross_stats["Sigma_VF"][k] = alpha * cross_stats["Sigma_VF"][k] + (1 - alpha) *(
            ((cross_stats["Sigma_VF"][k] + old_mean_V[k] * old_mean_F[k]) * new_Nk[k] + delta_Q_VF[k])
            / new_Nk[k] 
            - gmm_v.means_[k] * gmm_f.means_[k]
        )
...
def conditional_mean_f_given_x(cross_stats, gmm_x, gmm_f, f_x, source):
    ...
    B, dim_x = f_x.shape
    assert dim_x == expected_dim == gmm_f.d, (
        "Diagonal conditional recovery requires observed and fused feature "
        "dimensions to match"
    )
    K = gmm_f.K
    dim_f = gmm_f.d
    ...
    for k in range(K):
        ...
        # print(same)
        # Diagonal of Sigma_XF[k] = Cov(f_X, f_F | k), shape [dim_f]
        Sigma_XF_k = cross_stats[sigma_xf_key][k]

        cov_x_k = gmm_x.covariances_[k] + gmm_x.reg_covar  # [dim_x]

        diff_x = f_x - mu_x_k.unsqueeze(0)  # [B, dim_x]

        # Diagonal form of Sigma_FX @ Sigma_X^{-1} @ diff_x.
        delta_f = Sigma_XF_k.unsqueeze(0) * diff_x / cov_x_k.unsqueeze(0)
        
        cond_means[:, k, :] = mu_f_k.unsqueeze(0) + delta_f
...
def recovery_uncertainty_penalty(cross_stats, gmm_x, gmm_f, source, score_block=8):
    """
    Exact diagonal-covariance computation of
    ...
    for c in range(K):
        # Diagonal of Sigma_X,c^{-1} Sigma_XF,c.
        solved = cross_xf[c] / gmm_x.cholesky_L[c].square()  # [d]

        # Diagonal of the conditional covariance:
        # Sigma_F|X,c = Sigma_F,c
        #               - Sigma_FX,c Sigma_X,c^{-1} Sigma_XF,c.
        cond_cov = gmm_f.covariances_[c] - cross_xf[c] * solved  # [d]

        # k: class used by fused-view GDA for scoring.  Retain the
        # score_block loop so peak temporary memory stays bounded.
        for start in range(0, K, score_block):
            end = min(start + score_block, K)

            scoring_variances = gmm_f.cholesky_L[start:end].square()  # [block, d]
            penalty[c, start:end] = (
                0.5 * (cond_cov.unsqueeze(0) / scoring_variances).sum(dim=-1)
            )
...
def predict_x2f(cross_stats, gmm_x, gmm_f, f_x, source, temp=20, device=None):
    ...
    penalty = recovery_uncertainty_penalty(
        cross_stats, gmm_x, gmm_f, source, score_block=8
    )  # [K, K]
...
def recovery_reliability(
    ...
    cholesky_m: [C, d], diagonal Cholesky factors
    cov_f:      [C, d], diagonal fused-view covariances
    cross_mf:   [C, d], diagonal cross-covariances
    """
    ...
    for c in range(C):
        # Diagonal form of L_m^{-1} Sigma_mF.
        Y = cross_mf[c] / cholesky_m[c]

        explained_var = Y.square().sum()

        total_var = cov_f[c].sum()
...
@torch.no_grad()
def recovery_uncertainty_weight(
    cholesky_m,
    cov_f,
    cross_mf,
    device_out=None,
    ...
    Args:
        cholesky_m: [C, d]
            Diagonal Cholesky factors of Sigma_m.

        cov_f: [C, d]
            Diagonal covariance vectors of the fused view.

        cross_mf: [C, d]
            Diagonal cross-covariance Sigma_mF.
    ...
    for c in range(C):
        ...
        # Compute ||L_m^{-1} Sigma_mF||_F^2 by feature chunks.
        for start in range(0, d, chunk_size):
            end = min(start + chunk_size, d)

            rhs = cross_mf[c, start:end]
            Y = rhs / cholesky_m[c, start:end]

            explained_var += Y.float().square().sum()

        # Tr(Sigma_F)
        total_var = cov_f[c].float().sum().clamp_min(eps)
...
def forward_and_adapt(
    ...
    if has_full:
        update_cross_stats_from_full(
            ...
            ca=ca, cv=cv, feat_full=feat_full, gamma_full=pred[mask_full], alpha=args.alpha
        )

    logits_gmm = torch.zeros_like(outputs, dtype=torch.float32)
    # logits_gda = torch.zeros_like(outputs, dtype=torch.float32)
    # logits_recover = torch.zeros_like(outputs, dtype=torch.float32)
    ...
    if has_full:
        ...
        # logits_gda[mask_full] = logits_f
        # logits_recover[mask_full] = logits_f
        

    if mask_audio.sum() > 0:
        # logits_gmm[mask_audio] = predict_x2f(
        #     cross_stats=cross_stats, gmm_x=gmm_a, gmm_f=gmm_f, f_x=feat[mask_audio],
        #     source="a", device=pred.device,
        # ).float()
        # logits_gmm[mask_audio] = gmm_a.predict_batch(feat[mask_audio], device=pred.device)
        # rho = recovery_reliability(gmm_a.cholesky_L, gmm_a.covariances_, cross_stats["Sigma_AF"], pred.device)
        ...
        #             source="a", temp=args.temp, device=pred.device,).float() + args.beta * gmm_a.predict_batch(feat[mask_audio], device=pred.device)
               
        # logits_gmm[mask_audio] = (1 - args.beta) * predict_x2f(cross_stats=cross_stats, gmm_x=gmm_a, gmm_f=gmm_f, f_x=feat[mask_audio],
        #     source="a", temp=args.temp, device=pred.device,).float() + args.beta * gmm_a.predict_batch(feat[mask_audio], device=pred.device)
        # logits_gda[mask_audio] = gmm_a.predict_batch(feat[mask_audio], device=pred.device)
        # logits_recover[mask_audio] = predict_x2f(cross_stats=cross_stats, gmm_x=gmm_a, gmm_f=gmm_f, f_x=feat[mask_audio],
        #     source="a", temp=args.temp, device=pred.device,).float()
        # logits_gmm[mask_audio], _ = entropy_weighted_logits(predict_x2f(cross_stats=cross_stats, gmm_x=gmm_a, gmm_f=gmm_f, f_x=feat[mask_audio],
        #     source="a", temp=args.temp, device=pred.device,).float(), gmm_a.predict_batch(feat[mask_audio], device=pred.device), tau=50)
        # logits_gda = gmm_a.predict_batch(feat[mask_audio], device=pred.device)
        # logits_recover = predict_x2f(cross_stats=cross_stats, gmm_x=gmm_a, gmm_f=gmm_f, f_x=feat[mask_audio], source="a", device=pred.device,).float()
        # logits_gmm[mask_audio], _ = sample_count_weighted_logits(logits_gda, logits_recover, cross_stats['Nk'].sum(), args.n0)

    if mask_video.sum() > 0:
        # logits_gmm[mask_video] = predict_x2f(
        #     cross_stats=cross_stats, gmm_x=gmm_v, gmm_f=gmm_f, f_x=feat[mask_video],
        #     source="v", device=pred.device,
        # ).float()
        # logits_gmm[mask_video] = gmm_v.predict_batch(feat[mask_video], device=pred.device)
        # rho = recovery_reliability(gmm_v.cholesky_L, gmm_v.covariances_, cross_stats["Sigma_VF"], pred.device)
        weight =  recovery_uncertainty_weight(gmm_v.cholesky_L, gmm_v.covariances_, cross_stats["Sigma_VF"], pred.device)
        ...
        #     source="v", temp=args.temp, device=pred.device,).float() + args.beta * gmm_v.predict_batch(feat[mask_video], device=pred.device)
        # logits_gda[mask_video] = gmm_v.predict_batch(feat[mask_video], device=pred.device)
        # logits_recover[mask_video] = predict_x2f(cross_stats=cross_stats, gmm_x=gmm_v, gmm_f=gmm_f, f_x=feat[mask_video],
        #     source="v", temp=args.temp, device=pred.device,).float()
        # logits_gmm[mask_video], _ = entropy_weighted_logits( predict_x2f(cross_stats=cross_stats, gmm_x=gmm_v, gmm_f=gmm_f, f_x=feat[mask_video],
        #     source="v", temp=args.temp, device=pred.device,).float(), gmm_v.predict_batch(feat[mask_video], device=pred.device), tau=50)
        # logits_gda = gmm_v.predict_batch(feat[mask_video], device=pred.device)
        # logits_recover = predict_x2f(cross_stats=cross_stats, gmm_x=gmm_v, gmm_f=gmm_f, f_x=feat[mask_video], source="v", device=pred.device,).float()
        # logits_gmm[mask_video], _ = sample_count_weighted_logits(logits_gda, logits_recover, cross_stats['Nk'].sum(), args.n0)
