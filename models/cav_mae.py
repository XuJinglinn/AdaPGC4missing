...
class CAVMAEFT(nn.Module):
    ...
    def forward(self, a, v, mode):
        ...
        # used in case that the model is finetuned with both modality, but in inference only audio is given
            ...
            u = a
            for blk in self.blocks_u:
                u, _ = blk(u) # note here use unified normalization
            u = self.norm(u)
            u = u.mean(dim=1)
    ...
    def forward_eval_with_features(self, a, v):
        B = a.size(0)
        device = a.device
        embed_dim = self.mlp_head[-1].in_features
        out_dim   = self.mlp_head[-1].out_features

        eps = 0.0 
        a0 = a.flatten(1).abs().amax(dim=1) <= eps
        v0 = v.flatten(1).abs().amax(dim=1) <= eps

        both_missing = a0 & v0
        video_only   = a0 & ~v0   # audio missing, only video available
        audio_only   = v0 & ~a0   # video missing, only audio available
        full         = ~a0 & ~v0
        
        logits = torch.zeros(B, out_dim, device=device, dtype=torch.float16)
        feat   = torch.zeros(B, embed_dim, device=device, dtype=torch.float32)
            
        ca = cv = None  # only the 'full' branch may produce
        
        if video_only.any():
            # print('exist video only')
            x_v, f_v = self.forward_video_only(v[video_only]) 
            logits = torch.index_put(logits, (video_only,), x_v)
            feat[video_only]   = f_v

        if audio_only.any():
            # print('exist audio only')
            x_a, f_a = self.forward_audio_only(a[audio_only])
            logits = torch.index_put(logits, (audio_only,), x_a)
            feat[audio_only]   = f_a

        if full.any():
            x_f, ca, cv, f_f = self.forward_full(a[full], v[full])
            logits = torch.index_put(logits, (full,), x_f)
            feat[full]   = f_f

        if both_missing.any():
            raise NotImplementedError('Both audio and video are missing, this case is not supported in the current implementation.')
    ...
    def forward_full(self, a, v):
        ...
        feat = self.mlp_head[0](x)
        x = self.mlp_head[1](feat)
        # x = self.mlp_head(x)
        ...
        ca = self.norm_a(ca)
        ca = ca.mean(dim=1)
        ca = self.mlp_head[0](ca)

        for blk in self.blocks_u:
            cv, _ = blk(v)
        cv = self.norm_v(cv)
        cv = cv.mean(dim=1)
        cv = self.mlp_head[0](cv)

        return x, ca, cv, feat


    def forward_audio_only(self, a):
        a = a.unsqueeze(1)
        a = a.transpose(2, 3)
        a = self.patch_embed_a(a)
        a = a + self.pos_embed_a
